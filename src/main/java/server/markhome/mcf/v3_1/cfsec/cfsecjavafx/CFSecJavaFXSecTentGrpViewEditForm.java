// Description: Java 25 JavaFX View/Edit Form implementation for SecTentGrp.
...
/**
 *	CFSecJavaFXSecTentGrpViewEditForm JavaFX View/Edit Form implementation
 *	for SecTentGrp.
 */
public class CFSecJavaFXSecTentGrpViewEditForm
	...
	ICFForm
{
	...
	public CFSecJavaFXSecTentGrpViewEditForm( ICFFormManager formManager, ICFSecJavaFXSchema argSchema, ICFSecSecTentGrpObj argFocus, ICFFormClosedCallback closeCallback, boolean cameFromAdd ) {
		super();
		...
		if( ( argFocus != null ) && ( ! argFocus.getIsNew() ) ) {
			argFocus = (ICFSecSecTentGrpObj)argFocus.read( true );
			javaFXFocus = argFocus;
		}
		javafxViewEditPane = argSchema.getSecTentGrpFactory().newViewEditPane( cfFormManager, argFocus );
	...
	public void forceCancelAndClose() {
		...
		if( focus != null ) {
			ICFSecSecTentGrpEditObj editObj = (ICFSecSecTentGrpEditObj)focus.getEdit();
			if( editObj != null ) {
				if( editObj.getIsNew() ) {
					editObj.endEdit();
					editObj = null;
	...
	public void setJavaFXFocus( ICFLibAnyObj value ) {
		...
		((ICFSecJavaFXSecTentGrpPaneCommon)javafxViewEditPane).setJavaFXFocus( value );
		ICFSecSecTentGrpObj argFocus = (ICFSecSecTentGrpObj)value;
		if( ( argFocus != null ) && ( ! argFocus.getIsNew() ) ) {
			argFocus = (ICFSecSecTentGrpObj)argFocus.read( true );
		...
		if( hboxMenu == null ) {
			...
			buttonEdit.setOnAction( new EventHandler<ActionEvent>() {
				@Override public void handle( ActionEvent e ) {
					...
					try {
						if( getJavaFXFocusAsSecTentGrp() != null ) {
							if( null == getJavaFXFocusAsSecTentGrp().getEdit() ) {
			...
			buttonClose.setOnAction( new EventHandler<ActionEvent>() {
				@Override public void handle( ActionEvent e ) {
					...
					try {
						...
						cfFormManager.closeCurrentForm();
						if( formClosedCallback != null ) {
							focus = getJavaFXFocusAsSecTentGrp();
	...
	public void setPaneMode( CFPane.PaneMode value ) {
		...
		try {
			...
			((ICFSecJavaFXSecTentGrpPaneCommon)javafxViewEditPane).setPaneMode( value );
		}
		catch( Throwable t ) {
			super.setPaneMode( oldMode );
			((ICFSecJavaFXSecTentGrpPaneCommon)javafxViewEditPane).setPaneMode( oldMode );
			throw t;
		}
		adjustButtons();
	}
