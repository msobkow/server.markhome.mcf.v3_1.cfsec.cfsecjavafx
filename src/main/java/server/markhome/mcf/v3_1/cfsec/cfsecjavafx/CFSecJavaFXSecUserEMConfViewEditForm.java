// Description: Java 25 JavaFX View/Edit Form implementation for SecUserEMConf.
...
/**
 *	CFSecJavaFXSecUserEMConfViewEditForm JavaFX View/Edit Form implementation
 *	for SecUserEMConf.
 */
public class CFSecJavaFXSecUserEMConfViewEditForm
extends CFBorderPane
implements ICFSecJavaFXSecUserEMConfPaneCommon,
	ICFForm
{
	public static String S_FormName = "View/Edit Email confirmation";
	...
	public CFSecJavaFXSecUserEMConfViewEditForm( ICFFormManager formManager, ICFSecJavaFXSchema argSchema, ICFSecSecUserEMConfObj argFocus, ICFFormClosedCallback closeCallback, boolean cameFromAdd ) {
		super();
		...
		if( ( argFocus != null ) && ( ! argFocus.getIsNew() ) ) {
			argFocus = (ICFSecSecUserEMConfObj)argFocus.read( true );
			javaFXFocus = argFocus;
		}
		javafxViewEditPane = argSchema.getSecUserEMConfFactory().newViewEditPane( cfFormManager, argFocus );
	...
	public void forceCancelAndClose() {
		...
		if( focus != null ) {
			ICFSecSecUserEMConfEditObj editObj = (ICFSecSecUserEMConfEditObj)focus.getEdit();
			if( editObj != null ) {
				if( editObj.getIsNew() ) {
					editObj.endEdit();
					editObj = null;
	...
	public void setJavaFXFocus( ICFLibAnyObj value ) {
		...
		((ICFSecJavaFXSecUserEMConfPaneCommon)javafxViewEditPane).setJavaFXFocus( value );
		ICFSecSecUserEMConfObj argFocus = (ICFSecSecUserEMConfObj)value;
		if( ( argFocus != null ) && ( ! argFocus.getIsNew() ) ) {
			argFocus = (ICFSecSecUserEMConfObj)argFocus.read( true );
		...
		if( hboxMenu == null ) {
			...
			buttonEdit.setOnAction( new EventHandler<ActionEvent>() {
				@Override public void handle( ActionEvent e ) {
					...
					try {
						if( getJavaFXFocusAsSecUserEMConf() != null ) {
							if( null == getJavaFXFocusAsSecUserEMConf().getEdit() ) {
			...
			buttonClose.setOnAction( new EventHandler<ActionEvent>() {
				@Override public void handle( ActionEvent e ) {
					...
					try {
						...
						cfFormManager.closeCurrentForm();
						if( formClosedCallback != null ) {
							focus = getJavaFXFocusAsSecUserEMConf();
	...
	public void setPaneMode( CFPane.PaneMode value ) {
		...
		try {
			...
			((ICFSecJavaFXSecUserEMConfPaneCommon)javafxViewEditPane).setPaneMode( value );
		}
		catch( Throwable t ) {
			super.setPaneMode( oldMode );
			((ICFSecJavaFXSecUserEMConfPaneCommon)javafxViewEditPane).setPaneMode( oldMode );
			throw t;
		}
		adjustButtons();
	}
